...
const BuildForm = ({ service }) => {
  ...
  const indicatorQuestions = useSelector(
  ...
  useEffect(() => {
    if (id) {
      dispatch({
        payload: { ...form, title: form["form-title"] },
        type: "updateForm",
      });
    }
  }, [form, id]);
  ...
  const onFinish = () => {
    ...
    id
      ? reduxDispatch(
          updateForm(
            omit(state, ["customQuestionInput", "builderType"]),
            service,
            id,
            history
          )
  ...
  const handleChangeBuilderType = (_type) => {
    if (isEmpty(state?.program))
      return message.error("Please select a program");
  ...
  const menu = (
    <Menu style={{ display: "flex", flexDirection: "column" }}>
      {BUILDER_TYPES?.map(({ name, value }) => (
        <Menu.Item key={value} onClick={() => handleChangeBuilderType(value)}>
          {name}
        </Menu.Item>
      ))}
    </Menu>
          ...
          >
                ...
                <Col span={8}>
                  ...
                  >
                    <Select
                      onChange={(value) =>
                        dispatch({
                          payload: { name: "program", value: value.toString() },
                          type: "formData",
                        })
                      }
            ...
            <Card>
              <Dropdown trigger={["hover", "click"]} overlay={menu}>
                <Button style={{ marginBottom: 20 }} type="primary" ghost>
                  Create Question
                </Button>
              </Dropdown>
              ...
              {components?.map((component, idx) => {
                ...
                const handleSelectQuestion = (val) => {
                  if (val === "custom") {
                    ...
                    return dispatch({
                      payload: { value: true, id: idx },
                      type: "customQuestionInput",
                    });
                  }
                  dispatch({
                    payload: { value: false, id: idx },
                    type: "customQuestionInput",
                  });
                  change(
                    transformNonEventChange({
                      ...
                      <Row gutter={[16, 16]}>
                        <Col span={7}>
                          ...
                          >
                            ...
                            >
                              {indicators(
                                selectedProgramSdgs(+state?.program)
                              )?.map((indicator, idx) => (
                                <Option
                                  key={idx}
                                  value={indicator?.programIndicatorId}
                                >
                        ...
                        <Col span={8}>
                          ...
                          >
                            ...
                            >
                              ...
                              {indicatorQuestions?.map(({ question, id }) => (
                                <Option value={id}>{question}</Option>
                              ))}
                            </Select>
                          </Form.Item>
                        </Col>
                        {((customQuestionInput && customQuestionInput[idx]) ||
                          !isEmpty(question)) && (
                          <Col span={6}>
                            <Form.Item
